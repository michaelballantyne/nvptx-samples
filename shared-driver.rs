...
fn main(){
    unsafe {
        ...
        let ptxdata = File::open(&Path::new("copy.ptx")).read_to_end().unwrap();
        let kernel = "kernel";
        ...
        let mut hostNums = Vec::from_fn(16, |i| (i as i32) / 2 as i32);
	println!("{}", hostNums);

        checkCudaErrors(cuMemcpyHtoD(nums_dev, hostNums.as_ptr() as *libc::c_void, (std::mem::size_of::<i32>() * 16) as u64));

        let mut kernelParams: ~[&mut CUdeviceptr] = ~[&mut counts_dev];
	...
	let theptr = counts_dev as *libc::c_void;

        checkCudaErrors(cuLaunchKernel(function, gridSizeX, gridSizeY, gridSizeZ,
				       blockSizeX, blockSizeY, blockSizeZ,
                   0, ptr::null(), theptr as *libc::c_void, ptr::null()));
